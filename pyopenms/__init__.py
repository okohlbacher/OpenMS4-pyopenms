...
# Build-time identity of the core SDK used for this wheel. The Python package
# release and the linked scientific core have separate version identities.
...
if os.path.exists(default_openms_data_path):
    ...
    if not env_openms_data_path:
        # Check the versioned SDK data directory in this Python installation prefix
        from pathlib import Path
        _possible_paths = [
            Path(sys.prefix) / "share" / "OpenMS" / __openms_core_version__,
        ]
...
# package (share/OpenMS/openms_thermo_bridge/managed). The explicit managed-runtime
# override keeps the assemblies aligned with the wheel's bundled data. A value
# the user already set is respected; no override is set if no bridge is bundled.
...
# its dependencies next to the modules (found automatically), but an unrepaired
# standalone build uses the installed SDK's library directories.
# PYOPENMS_DLL_PATH (os.pathsep-separated) lets the caller point pyOpenMS
