...
Usage:
    PYTHONPATH=build/pyOpenMS python3 tests/benchmark_pyopenms.py
    PYTHONPATH=build/pyOpenMS python3 tests/benchmark_pyopenms.py --json results.json
    PYTHONPATH=build/pyOpenMS python3 tests/benchmark_pyopenms.py --quick
"""
...
def find_mzml_file_auto() -> Optional[str]:
    ...
    # Benchmarks use package-owned files and explicitly installed TOPP fixtures.
    topp_value = os.environ.get("OPENMS_TEST_DATA_PATH")
    topp_dir = Path(topp_value) if topp_value else None
    search_paths = [
        script_dir / "unittests" / "test2.mzML",
        script_dir / "unittests" / "test.indexed.mzML",
    ]
    if topp_dir is not None:
        search_paths.extend([
            topp_dir / "FeatureFinderMetabo_1_input.mzML",
            topp_dir / "OpenPepXL_input.mzML",
        ])
    ...
    for search_dir in [script_dir / "unittests"] + ([topp_dir] if topp_dir is not None else []):
